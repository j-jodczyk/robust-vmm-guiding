...
struct IncrementalCovariance2D<lightpmm::ParametricMixtureModel<TKernel, NKernels>>
{
    ...
    explicit IncrementalCovariance2D(Stream* stream)
    {
        stream->read(this, sizeof(IncrementalCovariance2D));
    }

    void serialize(Stream* stream) const
    {
        static_assert(std::is_trivially_copyable<IncrementalCovariance2D>::value, "not trivially serializable");

        stream->write(this, sizeof(IncrementalCovariance2D));
    }
    ...
    void updateStatistics(const TDistribution& distribution, const TSampleRange& samples)
    {
        ...
        for (uint32_t k=0; k<numActiveKernels; ++k)
        {
            ...
            varianceAndCovariance[k] = lightpmm::ifthen(newSumWeight > PMM_EPSILON, (varianceAndCovariance[k]*sumWeights[k]+batchUnnormalizedVariance[k])/newSumWeight, lightpmm::Vec<ScalarType>{0.0f, 0.0f, 0.0f});
    ...
    void updateStatisticsMasked(const TDistribution& distribution, const TSampleRange& samples, const std::array<typename ScalarType::BooleanType, TDistribution::NumKernels::value>& mask)
    {
        ...
        for (uint32_t k=0; k<numActiveKernels; ++k)
        {
            ...
            const ScalarType newSumWeight = sumWeights[k]+batchWeight[k];
            varianceAndCovariance[k] = lightpmm::ifthen(mask[k], lightpmm::ifthen(newSumWeight > PMM_EPSILON, (varianceAndCovariance[k]*sumWeights[k]+batchUnnormalizedVariance[k])/newSumWeight, lightpmm::Vec<ScalarType>{0.0f, 0.0f, 0.0f}), varianceAndCovariance[k]);
            sumWeights[k] = lightpmm::ifthen(mask[k], newSumWeight, sumWeights[k]);
        }
    }

    Matrix2x2 computeCovarianceMatrix(uint32_t component) const
    {
        const float componentSumWeights = sumWeights[component/ScalarType::Width::value][component%ScalarType::Width::value];
        const Vector variance{EXPECT_TAKEN(componentSumWeights > 0.0f) ? varianceAndCovariance[component/ScalarType::Width::value][component%ScalarType::Width::value] : Vector{0.0f}};
        return Matrix2x2{variance.x, variance.z, variance.z, variance.y};
    }
    ...
    std::string toString() const
    {
        ...
        oss << "IncrementalCovariance2D [\n";
        for (size_t i=0; i<TDistribution::MaxK::value; ++i)
        {
            const float weight = sumWeights[i/ScalarType::Width::value][i%ScalarType::Width::value];
            const Vector variance = (weight > 0.0f) ? varianceAndCovariance[i/ScalarType::Width::value][i%ScalarType::Width::value] : Vector{0.0f};
            oss << "  [" << i << "]: variance = " << variance.toString()<< ", sumWeights = " << weight << '\n';
        }
        oss << ']';
