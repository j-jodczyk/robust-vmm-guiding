...
struct IncrementalDistance<lightpmm::ParametricMixtureModel<TKernel, NKernels>>
{
    ...
    explicit IncrementalDistance(Stream* stream)
    {
        stream->read(this, sizeof(IncrementalDistance));
    }

    void serialize(Stream* stream) const
    {
        static_assert(std::is_trivially_copyable<IncrementalDistance>::value, "not trivially serializable");

        stream->write(this, sizeof(IncrementalDistance));
    }
    ...
    void updateDistances(const TDistribution& distribution, const TSampleRange& samples)
    {
        ...
        for (const auto& sample : samples)
        {
            ...
            {
                SLog(EWarn, "invalid sample distance %f. skipping sample.", sample.distance);
                continue;
            }
            ...
            for (uint32_t i=0; i<(numComponents+ScalarType::Width::value-1)/ScalarType::Width::value; i++)
            {
            ...
            {
                const ScalarType sampleWeightTimesSoftAssignmentTimesPDF = weightedComponentPDFs[k]*componentPDFs[k]*sampleWeightDivMixturePDF;
    ...
    void reposition(TDistribution& distribution, const Vector newPosToOldPos)
    {
        ...
        for (uint32_t k=0; k<(distribution.getK()+ScalarType::Width::value-1)/ScalarType::Width::value; k++)
        {
            typename TDistribution::KernelType& kernel = distribution.getComponent(k);

            const typename ScalarType::BooleanType isInfiniteOrInvalid = lightpmm::is_inf(distances[k]) || sumWeights[k] <= 0.0f;
            ...
            kernel.m_mu  = lightpmm::ifthen(isInfiniteOrInvalid, kernel.m_mu,  newPosToLightSource/newPosToLightSourceDistance);
        }
    }
    ...
    void merge(const TDistribution& distribution, const uint32_t componentA, const uint32_t componentB)
    {
        ...
        const float distanceA = distances[componentA/ScalarType::Width::value][componentA%ScalarType::Width::value];
        const float distanceB = distances[componentB/ScalarType::Width::value][componentB%ScalarType::Width::value];
        //using harmonic mean
        const float mergedDistance = mergedWeight/(weightA/distanceA+weightB/distanceB);

        distances[componentA/ScalarType::Width::value].insert(componentA%ScalarType::Width::value, mergedDistance);
        ...
        {
            distances[componentB/ScalarType::Width::value].insert(componentB%ScalarType::Width::value, distances[lastComponent/ScalarType::Width::value][lastComponent%ScalarType::Width::value]);
    ...
    void split(const TDistribution& distribution, const uint32_t component)
    {
        ...
        distances[newComponent/ScalarType::Width::value].insert(newComponent%ScalarType::Width::value, distances[component/ScalarType::Width::value][component%ScalarType::Width::value]);
        const float splitWeight = 0.5f*sumWeights[component/ScalarType::Width::value][component%ScalarType::Width::value];
        sumWeights[component/ScalarType::Width::value].insert(component%ScalarType::Width::value, splitWeight);
        sumWeights[newComponent/ScalarType::Width::value].insert(newComponent%ScalarType::Width::value, splitWeight);
    }
    ...
    std::string toString() const
    {
        ...
        {
            oss << "  [" << i << "]:"
                << " distance = " << (sumWeights[i/ScalarType::Width::value][i%ScalarType::Width::value] > 0.0f ? distances[i/ScalarType::Width::value][i%ScalarType::Width::value] : std::numeric_limits<float>::infinity())
