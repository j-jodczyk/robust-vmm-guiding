...
struct IncrementalPearsonChiSquared<lightpmm::ParametricMixtureModel<TKernel, NKernels>>
{
    ...
    explicit IncrementalPearsonChiSquared(Stream* stream)
    {
        stream->read(this, sizeof(IncrementalPearsonChiSquared));
    }

    void serialize(Stream* stream) const
    {
        static_assert(std::is_trivially_copyable<IncrementalPearsonChiSquared>::value, "not trivially serializable");

        stream->write(this, sizeof(IncrementalPearsonChiSquared));
    }
    ...
    std::string toString() const
    {
        ...
        oss << "IncrementalPearsonChiSquared [\n";
        for (size_t i=0; i<TDistribution::MaxK::value; ++i)
        {
            oss << "  [" << i << "]:"
                << " divergencePlusOneTimesIntegralSqr = " << divergencePlusOneTimesIntegralSqr[i/ScalarType::Width::value][i%ScalarType::Width::value]
                << ", numSamples = " << numSamples[i/ScalarType::Width::value][i%ScalarType::Width::value] << '\n';
        }
        oss << ']';
