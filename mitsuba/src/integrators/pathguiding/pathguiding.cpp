...
public:
        ...
        : MonteCarloIntegrator(props)
    {
        ...
        Log(EInfo, "m_renderMaxSeconds is set to %i", m_renderMaxSeconds);
        ...
        if (m_trainingSamples == 0)
            m_trainingSamples = 32;
    ...
    void iterationPostprocess(const ref<Film> film, uint32_t numSPP, const RenderJob *job)
    {
        ...
        if (m_training && !m_canceled)
        {
            ...
            if (EXPECT_TAKEN(numValidSamples >= m_minSamplesToStartFitting))
            {
                ...
                m_guidingField.updateField(samples, zeroValuedSamples, job); // HERE is where the *magic* happens (EM)
